...
export function Jewelry3DLogo({ width = 350, height = 300, idPrefix = "logo" }: Jewelry3DLogoProps) {
  const [isHovered, setIsHovered] = useState(false)

  const g1 = `${idPrefix}-gradient1`
  const g2 = `${idPrefix}-gradient2`
  const g3 = `${idPrefix}-gradient3`
  const strokeGrad = `${idPrefix}-stroke-gradient`
  const sparkleGrad = `${idPrefix}-sparkle-gradient`
    ...
    >
      ...
      <div
        className={`absolute inset-0 rounded-full transition-all duration-700 ${
          isHovered
            ? "blur-3xl bg-linear-to-r from-amber-400 via-yellow-300 to-orange-400 scale-150 opacity-80"
            : "blur-2xl bg-amber-500/30 scale-100 opacity-40"
        }`}
      ></div>

      {/* Rotating Ring Effects */}
      <div className={`absolute inset-0 transition-all duration-1000 ${isHovered ? "animate-spin-slow" : ""}`}>
        <div
          className="absolute inset-0 border-2 border-amber-400/40 rounded-full"
          style={{
      ...
      <div className={`absolute inset-0 transition-all duration-1000 ${isHovered ? "animate-spin-reverse" : ""}`}>
        <div
      ...
      <svg
        ...
        height={height}
        viewBox="0 0 200 200"
        className={`relative z-10 transition-all duration-700 ${isHovered ? "scale-110" : "scale-100"}`}
        style={{
          filter: isHovered
      ...
      >
        ...
        <g className={`transition-all duration-700 ${isHovered ? "animate-pulse-glow" : ""}`}>
          {/* Top triangle */}
          <path
            d={isHovered ? "M 100 30 L 140 70 L 60 70 Z" : "M 100 40 L 130 70 L 70 70 Z"}
            fill={`url(#${g1})`}
          ...
          {/* Bottom triangle */}
          <path
            d={isHovered ? "M 50 80 L 100 130 L 150 80 L 100 170 Z" : "M 60 80 L 100 120 L 140 80 L 100 160 Z"}
            fill={`url(#${g3})`}
            stroke={`url(#${strokeGrad})`}
            strokeWidth="2"
            className="transition-all duration-700"
      ...
      {isHovered && (
        <>
          <div
            className="absolute top-1/2 left-1/2 w-2 h-2 bg-amber-300 rounded-full animate-orbit-1"
            style={{ marginLeft: "-4px", marginTop: "-4px" }}
          ></div>
          <div
            className="absolute top-1/2 left-1/2 w-2 h-2 bg-yellow-300 rounded-full animate-orbit-2"
            style={{ marginLeft: "-4px", marginTop: "-4px" }}
          ></div>
          <div
            className="absolute top-1/2 left-1/2 w-2 h-2 bg-orange-300 rounded-full animate-orbit-3"
            style={{ marginLeft: "-4px", marginTop: "-4px" }}
          ></div>
        </>
      )}
